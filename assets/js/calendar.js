...
jQuery(document).ready(function ($) {
  ...
  function wpestate_allinone_owner_insert_customprice_internal() {
    var period_extra_price_per_guest,
      period_price_per_weekeend,
    ...
    period_extra_price_per_guest = jQuery(
      "#period_extra_price_per_guest"
    ).val();
    period_price_per_weekeend = jQuery("#period_price_per_weekeend").val();
    period_checkin_change_over = jQuery("#period_checkin_change_over").val();
    period_checkin_checkout_change_over = jQuery(
      "#period_checkin_checkout_change_over"
    ).val();
  ...
  var curent_id;
  $(".booking-calendar-wrapper-allinone .has_future").on(
    "click",
    function (event) {
      ...
      if (
        $(this).hasClass("calendar-reserved") ||
        $(this).hasClass("pick_block_dates")
      ) {
        // click on a booked spot
        return;
      } else {
      ...
      if (all_calendar_click === 0) {
        ...
        $(
          ".booking-calendar-wrapper-allinone .has_future[data-curent-id!=" +
            curent_id +
            "]"
        ).addClass("pick_block_dates");
      } else {
        ...
        $(".booking-calendar-wrapper-allinone .calendar-selected").removeClass(
          "calendar-selected "
        );
        $(".booking-calendar-wrapper-allinone .has_future").removeClass(
          "pick_block_dates calendar-reserved-stop calendar-reserved-start"
        );
  ...
  function allinone_mark_as_booked(parent, start_date, end_date, curent_id) {
    ...
    jQuery("#start_date_owner_book").val(
      wpestate_convert_selected_days_reverse(
        wpestate_timeConverter_noconver(start_date)
      )
    );
    jQuery("#end_date_owner_book").val(
      wpestate_convert_selected_days_reverse(
        wpestate_timeConverter_noconver(end_date)
      )
    );
  ...
  function wprentals_google_fillInAddress(place) {
    ...
    var map_city = new google.maps.Map(
      document.getElementById("property_country"),
      {
        center: pyrmont,
        zoom: 15,
      }
    ...
    $("#set_price_dates").on("click", function () {
      ...
      jQuery.ajax({
        ...
        success: function (data) {
          ...
          jQuery(
            ".booking-calendar-wrapper-in-price .calendar-reserved-price .price-day"
          ).remove();
          jQuery(
            ".booking-calendar-wrapper-in-price .calendar-reserved-price .custom_set_price"
          ).remove();
          jQuery(
            ".booking-calendar-wrapper-in-price .calendar-reserved-price"
          ).append('<span class="custom_set_price">' + data + "</span>");
          jQuery(
            ".booking-calendar-wrapper-in-price .calendar-reserved-price"
          ).removeClass(
            "calendar-reserved-price,calendar-reserved-start-price,calendar-reserved-stop-price"
          );
          jQuery("#book_dates").empty().text(ajaxcalls_vars.reserve);
          jQuery("#book_notes").val("");
          jQuery(".booking-calendar-wrapper-in-price td").removeClass(
  ...
  $(".booking-calendar-wrapper-in .has_future").on("click", function (event) {
    ...
    if (calendar_click === 0) {
      ...
      $(".booking-calendar-wrapper-in .calendar-selected").removeClass(
        "calendar-selected"
      );
  ...
  $(".booking-calendar-wrapper-in-price .has_future").on("click", function () {
    ...
    if (calendar_click_price === 0) {
      ...
      $(".booking-calendar-wrapper-in-price .calendar-selected").removeClass(
        "calendar-selected"
      );
  ...
  $("#edit_prop_ammenities").on("click", function () {
    ...
    $(".feature_list_save").each(function () {
      if ($(this).prop("checked")) {
        custom_fields_amm =
          custom_fields_amm + $(this).attr("data-feature") + "~";
      }
    });
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.edited) {
          $("#profile_message")
            .empty()
            .append('<div class="login-alert">' + data.response + "<div>");
        } else {
          $("#profile_message")
            .empty()
            .append('<div class="login-alert">' + data.response + "<div>");
  ...
  $("#edit_prop_details").on("click", function () {
    ...
    var children_allowed = jQuery(
      "input[name='children_allowed']:checked"
    ).val();
  ...
  $("#edit_prop_price").on("click", function () {
    ...
    checkin_checkout_change_over = jQuery(
      "#checkin_checkout_change_over"
    ).val();
  ...
  if (ajaxcalls_add_vars.mandatory_fields.constructor === Array) {
    ajaxcalls_add_vars.mandatory_fields.forEach(function (field) {
      ...
      if (jQuery("#" + field).length > 0) {
        ...
        if (
          field === "prop_category_submit" ||
          field === "prop_action_category_submit"
        ) {
        ...
        if (
          field == "checkin_change_over" ||
          field == "checkin_checkout_change_over"
        ) {
